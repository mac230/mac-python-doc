...
(defun r-python-shell-or-ielm-send-region ()
  ...
  (let* ((w (window-list))
    ...
    (cond
     ;; ESS 
     ((or (member 'inferior-ess-mode modes)
          (member 'inferior-ess-r-mode modes))
...
commenting out the ibuffer line."
  ...
  (let* ((curr-buff (current-buffer))
	...
	(insert
     ...
     ;; if no date line is there (i.e., haven't called the func yet today),
...
(defun time-hydra (&optional arg)
  "My re-written function to track time on tasks.  You can supply a
time (in minutes) or use keys to input commonly used values:
j = 35 minutes
f = 40 minutes
g = 50 minutes
c = custom amount (read from minibuffer)"
  (interactive)
  (if (not arg)
    (let ((arg (read-key "Which key: "))
          (time))
      (cond
       ;; 'a' - 20 minutes
       ((= arg 97)
       ...
       ((= arg 99)
        (progn
          (global-data-entry-mode 1)
          (setq time (read-string "Enter time (minutes - de on): "))
          (global-data-entry-mode -1)
          (pmdr-timer nil time)))
       ;; 'i' - 'on-a-timer'
       ((= arg 105)
       ...
       ((= arg 116)
        (what-time-is-it))
       ;; arg other than above - prompt for a custom value
       (t
        (progn
          (global-data-entry-mode 1)
          (setq time (read-string "Enter time (minutes - de on): "))
    ...
    (if (number-or-marker-p arg)
        (pmdr-timer nil (format "%s" arg))
      (message "Function supplied with non-numeric arg."))
      ))
